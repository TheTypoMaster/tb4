...
function chkPass(pwd) {
	...
	/* Modify overall score value based on usage vs requirements */
		...
		oScorebar.style.backgroundPosition = "-" + parseInt(nScore*3 * 4) + "px";
...
function togPwdMask() {
	...
	var oPwd2 = $("password2");
	var oTxt2 = $("password2Txt");
	var oMask = $("mask");
	if (oMask.checked) { 
		oPwd.value = oTxt.value;
		oPwd.className = ""; 
		oTxt.className = "dispnone"; 
		oPwd2.value = oTxt2.value;
		oPwd2.className = ""; 
		oTxt2.className = "dispnone"; 
	} 
	else { 
		oTxt.value = oPwd.value;
		oPwd.className = "dispnone"; 
		oTxt.className = "";
		oTxt2.value = oPwd2.value;
		oPwd2.className = "dispnone"; 
		oTxt2.className = "";
	}
}
...
function initPwdChk(restart) {
	...
	for (var i in arrZeros) { if($(arrZeros[i]) != null ) {$(arrZeros[i]).innerHTML = "0";}}
	for (var i in arrPassPars) { if($(arrPassPars[i]) != null ) {$(arrPassPars[i]).parentNode.className = "pass"; }}
	for (var i in arrPassDivs) { if($(arrPassDivs[i]) != null ) {$(arrPassDivs[i]).className = "pass"; }}
	for (var i in arrFailPars) { if($(arrFailPars[i]) != null ) {$(arrFailPars[i]).parentNode.className = "fail"; }}
	for (var i in arrFailDivs) { if($(arrFailDivs[i]) != null ) {$(arrFailDivs[i]).className = "fail"; }}
	$("scorebar").style.backgroundPosition = "0";
	if (restart) {
		$("password").className = "";
		$("passwordTxt").className = "dispnone";
		$("mask").checked = true;
	}
}
