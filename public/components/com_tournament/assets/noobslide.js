...
var noobSlide = new Class({
	...
	initialize: function(params){
		...
		this.replayMode = params.replayMode || 'rewind';
		if (this.replayMode == 'continuous'){
			this.items.include(this.items.length + 1);
		}
		...
		if (this.replayMode == 'continuous'){
			var first_element = new Element('span');
			first_element.injectInside('box1').setHTML(this.box.getChildren('span')[0].innerHTML);
		}
	...
	next: function(manual){
		...
		this.walk(null,manual);
		if(this.replayMode == 'continuous' && this.nextIndex == 1) { this.continuousPlay(); }
	},
	...
	continuousPlay: function() {
		...
		this.previousIndex = null;
		this.nextIndex = null;
		this.box.setStyle(this.modes[this.mode][0],'0px');
