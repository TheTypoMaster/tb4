...
class race_wizard_detailModelrace_wizard_detail extends JModel
{
	/**
  ...
  /**
	...
	function getTournamentVals() {
		...
		$query  = " SELECT * ";
		$query .= " FROM jos_ucbetman_tournament_buyins ORDER BY tournament_value ASC";
		$db->setQuery($query);
	...
	function getTotalBetters($meetingID) {
		...
		$query = "SELECT distinct `user_id` FROM `jos_ucbetman_tournament_bet_parent`  WHERE `tab_race_id` LIKE '%$meetingID%' ";
		$db->setQuery($query);
		$result = $db->LoadObjectList();
	...
	function _loadData()
	{
		...
		if (empty($this->_data))
		{
			$query = 'SELECT *, t.id as tournID, t.published as tournPub FROM jos_ucbetman_tournaments AS t ';
