<?php
/**
 * Joomla! 1.5 component withdrawalrequest
 *
...
class TournamentdollarsModelTournamenttransaction extends JModel
{
	...
	*/
	private function _buildQuery()
	{
		...
		$query = "SELECT t.id,t.amount,t.notes,t.created_date, r.name as recipient, r.id as recipient_id, r.username as recipient_username,"
...
//		print ($query);exit;
	...
	*/
	private function _buildQueryOrderBy()
	{
		...
		$orders = array('id', 'recipient', 'giver', 'type', 'amount', 'created_date', 'giver_id', 'recipient_id','giver_username', 'recipient_username');
	...
	*/
	private function _buildQueryWhere()
	{
		...
		if ($filter_search = trim($filter_search))
		{
			...
			$orcond[] = 'LOWER(g.username) LIKE "%'.$filter_search.'%"';
		...
		{
			$where[] = ' t.created_date >= "' . $filter_from_date . ' 00:00:00"';
		}
		
		if( $filter_to_date && preg_match('/^([0-9]{4})-([0-9]{2})-([0-9]{2})$/', $filter_to_date, $m) )
		{
			$where[] = ' t.created_date <= "' . $filter_to_date . ' 23:59:59"';
		}
		
		if( $filter_from_amount && ctype_digit($filter_from_amount))
		{
			$where[] = ' abs(t.amount/100) >= "'. $filter_from_amount .'"';
		}
		
		if( $filter_to_amount && ctype_digit($filter_to_amount))
		{
			$where[] = ' abs(t.amount/100) <= "'. $filter_to_amount .'"';
		}
		
		if( $filter_transaction_type )
		{
			$where[] = 't.tournament_transaction_type_id = "' . $filter_transaction_type . '"';
		}
	...
	*/
    function getTotalPage()
    {
		...
		{
			$query = "SELECT COUNT(*) AS tot FROM " . $db->nameQuote('#__tournament_transaction');
			//$query = $this->_buildQuery();
	...
	*/
	function store( $params )
	{
		...
		$transactionTypeId = $db->quote($params['tournament_transaction_type']);

		$table = $db->nameQuote('#__tournament_transaction');
    	$insertQuery = "INSERT INTO $table
			( recipient_id, giver_id, session_tracking_id, tournament_transaction_type_id, amount, notes, created_date )
	...
	*/
	...
	{
		$db =& $this->getDBO();
		$table = $db->nameQuote('#__tournament_transaction_type');
	...
	function getUserIdFromUsername( $username )
	{
		...
		$query = 'SELECT id FROM ' . $table . 'WHERE username = ' . $db->quote($username) . 'LIMIT 1';
		$db->setQuery($query);
		$rec = $db->loadObject();
		return $rec->id;
	}
