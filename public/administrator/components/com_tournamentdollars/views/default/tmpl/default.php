...
<form action="index.php?option=com_tournamentdollars" method="post" name="adminForm">

<table>
	<tr>
		<td align="left">
		<?php echo JText::_('Filter'); ?>:
			<input type="text" name="filter_tournament_search" id="search" value="<?php echo $this->lists['search'];?>" class="text_area" onchange="document.adminForm.submit();" />
		</td>
		<td>
			<select name="filter_tournament_transaction_type" onChange="document.adminForm.submit()">
<?php
...
?>
		...
		<td>
			<input type="text" value="<?php echo $this->lists['from_date'];?>" class="DatePicker" name="filter_tournament_from_date" id="from_date" alt="{format:'yyyy-mm-dd',yearStart:2010}" />
		</td>
		<td>
			<?php echo JText::_('To'); ?>:
		</td>
		<td>
			<input type="text" value="<?php echo $this->lists['to_date'];?>" class="DatePicker" name="filter_tournament_to_date" id="to_date" alt="{format:'yyyy-mm-dd',yearStart:2010}" />
		</td>
		<td>
			<?php echo JText::_('From'); ?>:
		</td>
		<td>
			$<input type="text" value="<?php echo $this->lists['from_amount'];?>" name="filter_tournament_from_amount" id="from_amount" size="5" onkeypress="return isNumberKey(event)" />
		</td>
		<td>
			<?php echo JText::_('To'); ?>:
		</td>
		<td>
			$<input type="text" value="<?php echo $this->lists['to_amount'];?>" name="filter_tournament_to_amount" id="to_amount" size="5" onkeypress="return isNumberKey(event)" />
		</td>
		<td>
			<button onclick="this.form.submit();">
				<?php echo JText::_('Search'); ?>
			</button>
			<button onclick="document.adminForm.filter_tournament_search.value='';
				document.adminForm.filter_tournament_transaction_type.value='';
				document.adminForm.filter_tournament_from_date.value='';
				document.adminForm.filter_tournament_to_date.value='';
				document.adminForm.filter_tournament_from_amount.value='';
				document.adminForm.filter_tournament_to_amount.value='';
				this.form.submit();">
...
?>
		<td>
			&nbsp;&nbsp;|&nbsp;&nbsp;
			<a href="index.php?option=com_tournamentdollars&amp;task=csv_export"><img src="/administrator/components/com_payment/images/export_icon.png" alt="Export Transactions" width="16" height="16" /> export</a>
		</td>
<?php
}
?>
...
<table class="adminlist">
	...
	<tr>
		...
		<th width="10%">
			<?php echo JHTML::_('grid.sort', JText::_('Giver Useranme'), 'giver_username', $this->lists['order_Dir'], $this->lists['order'] ); ?>
		</th>
		<th>
