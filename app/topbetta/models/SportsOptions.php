...
class SportsOptions extends \Eloquent {
	...
	public function getOptions($eventId, $typeId) {
		...
		return \DB::table('tbdb_market_type AS mt')
            ...
            ->where('sp.win_odds', '>', '1')
            ->select('m.market_status AS market_status','s.image_url AS image_url', 's.name AS bet_selection', 'sp.win_odds AS odds', 's.bet_place_ref', 's.bet_type_ref', 's.external_selection_id', 's.id AS selection_id', 'sp.line as line', 'm.id as type_id', 'm.event_id as event_id')->get();
