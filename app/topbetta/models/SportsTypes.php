...
class SportsTypes extends \Eloquent {
	...
	public function getTypes($eventId) {
		...
		$query = "SELECT DISTINCT(mt.id) AS id, mt.name AS bet_type, m.market_status as status FROM tbdb_market_type AS mt INNER JOIN tbdb_market AS m ON mt.id = m.market_type_id WHERE m.event_id = $eventId";
