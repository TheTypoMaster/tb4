	...
	public function getFormForRunnerAndRaceId($runner, $raceId) {
		...
		if(isset($runnersForm[0])){
			...
			foreach ($runnersForm[0]->last_starts as $last_starts){
				$data['detailed_form']['last_starts'][] = array('id' => (int)$last_starts->id, 'finish_position' => (int)$last_starts->finish_position, 'race_starters' => (int)$last_starts->race_starters, 'abr_venue' => $last_starts->abr_venue, 'race_distance' => $last_starts->race_distance,
