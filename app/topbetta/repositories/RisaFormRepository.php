...
class RisaFormRepository {
	...
	public function getFormForRunnerAndRaceId(&$runner, $raceId) {
		...
		if(isset($runnersForm[0])){
			$runner['detailed_form'] = array ('id'=> (int)$runnersForm[0]->id, 'age' => $runnersForm[0]->age, 'colour' => $runnersForm[0]->colour, 'sex' => $runnersForm[0]->sex, 'career' => $runnersForm[0]->career_results,
			...
			foreach ($runnersForm[0]->last_starts as $last_starts){
				$runner['detailed_form']['last_starts'][] = array('id' => (int)$last_starts->id, 'finish_position' => (int)$last_starts->finish_position, 'race_starters' => (int)$last_starts->race_starters, 'abr_venue' => $last_starts->abr_venue, 'race_distance' => $last_starts->race_distance,
