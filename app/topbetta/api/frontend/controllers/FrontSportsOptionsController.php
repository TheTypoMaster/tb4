		...
		// return \Cache::remember('sportsOptions-' . $eventId . '-' . $typeId, .2, function() use (&$eventId, &$typeId) {
			...
			if (count($options) > 0) {
				...
				foreach ($options as $option) {
					$eachOption[] = array('bet_selection' => $option -> bet_selection, 'odds' => (float)$option -> odds, 'bet_place_ref' => (int)$option -> bet_place_ref, 'external_selection_id' => (int)$option -> external_selection_id, 'id' => (int)$option -> selection_id, 'line' => $option -> line, 'bet_limit' => $betLimitValue, 'type_id' => (int)$option->type_id, 'event_id' => (int)$option->event_id);
				}
