...
class Bookmaker	{
	...
	*/
	...
	//const RACELIST_JSON 	= 'http://localhost/racelist.json';
	...
	* 
	*/
	public function emulateCSV($output_type=null){
		error_reporting('E_ALL');		
		
		echo "Downloading Race Data.. Please Wait.\n";
			...
			foreach($ordered_meetings as $details){
			     // if(strtoupper($details['meeting']['venue']) != strtoupper("Mornington"))
			      ...
			      foreach($details['raceList'] as $race) { 
					// Race line
					$details_list .= "R".$race['RaceNum'].",".date("Y-m-d H:i:s", strtotime($race['OutcomeDateTime'])).".0".",".strtoupper($race['Status']).",".$race['EventID']."\n";
					$w_odds = null;
					$p_odds = null;
					$w_results = null;
					$p_results = null;
